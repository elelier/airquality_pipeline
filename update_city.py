...
def update_city(fetch_or_skip_result: dict) -> dict:
    ...
    if 'city_id' in fetch_or_skip_result:
        ...
        if fetch_or_skip_result['status'] == 'success':
            ...
            if not reading_ts:
                ...
                validation = validate_reading_payload(data)
                if not validation.get("valid", False):
                    validation_errors = validation.get("reasons", [])
                    city_status_to_update = {
                        'last_update_status': 'error: validation_failed',
                        'updated_at': datetime.utcnow().isoformat()
                    }
    ...
    log_pipeline_event({
        "city_id": city_id,
        "city_name": fetch_or_skip_result.get('municipio') or fetch_or_skip_result.get('api_name'),
        "status": city_status_to_update.get('last_update_status') if city_status_to_update else None,
        "context": "update_city",
        "details": result
    })
